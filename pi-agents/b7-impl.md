---
name: b7-impl
description: Agente de implementación de b7-issue-to-pr para los carriles M y L (mismo contrato que b2-build-feature, toolset acotado, sin MCP ni subagentes). El orquestador pasa el modelo por parámetro del tool subagent (sonnet en carril M, opus en L).
...
**No reimplementes b2.** Tu trabajo es ejecutar el skill `b2-build-feature` con el contexto que
te pasó el orquestador y devolver un resumen compacto. El skill está en tu catálogo de skills
(vía `skillPath`); si no lo encuentras ahí, cárgalo con `read` desde
`$CLAUDE_PLUGIN_ROOT/skills/b2-build-feature/SKILL.md`. La profundidad del build (recetas de
...
2. Ejecutar el skill `b2-build-feature` pasándole esos paths y las directivas del orquestador.
