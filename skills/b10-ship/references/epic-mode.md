...
- **Carril b11 explícito:** los sub-issues cuyo reconcile trae `lane=b11` (label `lane:b11` de b0: `kind:docs|tests|infra`, `## Archivos previstos` ≤ 6, sin `src/routes/` ni migraciones con lógica) van por `references/lane-b11.md` — NUNCA por b7, b8 ni wave-build. **La SPEC nunca la escribe un fork**: agente `general-purpose` nuevo con prompt acotado (un fork hereda el contexto del epic y lo engorda — issue #57). `LANE_FALLBACK=b7` devuelve el issue al build normal de su ola.
